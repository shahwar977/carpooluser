...
class DataParser {
    ...
    fun parsetad(jObject: JSONObject): List<List<HashMap<String, String>>> {

        val routes = ArrayList<List<HashMap<String, String>>>()
        val jRoutes: JSONArray
        var jLegs: JSONArray
        val jSteps: JSONArray
        try {
            jRoutes = jObject.getJSONArray("routes")
            /** Traversing all routes  */

            val directionandduration = ArrayList<HashMap<String,String>>()
            for (i in 0 until jRoutes.length()) {
                jLegs = (jRoutes.get(i) as JSONObject).getJSONArray("legs")

                val googleDirectionsMap = HashMap<String, String>()

                val duration = jLegs.getJSONObject(i).getJSONObject("duration").getString("text")
                val distance = jLegs.getJSONObject(i).getJSONObject("distance").getString("text")
                googleDirectionsMap["duration"] = duration
                googleDirectionsMap["distance"] = distance
                directionandduration.add(googleDirectionsMap)
                routes.add(directionandduration)
            }
